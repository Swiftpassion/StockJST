...
# --- 3.1 อ่าน Master Stock (แก้ไข NameError) ---
...
def get_stock_from_sheet():
    try:
        ...
        col_map = {
            ...
            'รูป': 'Image', 'รูปภาพ': 'Image',
            'Stock': 'Initial_Stock', 'จำนวน': 'Initial_Stock',
            'Min_Limit': 'Min_Limit', 'Type': 'Product_Type', 'หมวดหมู่': 'Product_Type'
        }
...
# --- Functions for Saving Data (แก้ไข Range เป็น W) ---
...
def save_po_edit_split(row_index, current_row_data, new_row_data):
    try:
        ...
        # Update row เดิม (เปลี่ยน Range เป็น W)
...
with tab2:
    ...
    if not df_po.empty and not df_master.empty:
        ...
        # Merge กับ Master เพื่อเอาชื่อและรูป (เลือกเฉพาะ Columns ที่มีอยู่จริง)
        master_cols = [c for c in ['Product_ID', 'Product_Name', 'Image', 'Product_Type'] if c in df_master.columns]
        df_display = pd.merge(df_po_filter, df_master[master_cols], on='Product_ID', how='left')
