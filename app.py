    ...
    with st.container(border=True):
        ...
        with col_right_form:
            with st.form("po_form", border=False):
                ...
                # --- [แก้ไข] Logic การคำนวณและป้องกัน Error Mixed Type ---
                # 1. คำนวณยอดรวม (ใช้ or 0.0 เพื่อให้เป็น float เสมอ)
                calc_total = (qty_ord or 0.0) * (p_1688_ship or 0.0)
                
                # 2. เช็คว่ามีค่าเดิมจาก Database หรือไม่
                saved_total = val_num("Total_Yuan", default=0.0)

                # 3. เลือกค่าที่จะแสดง (ถ้ามีค่าเดิมใช้ค่าเดิม ถ้าไม่มีใช้ที่คำนวณ)
                display_total_value = saved_total if saved_total > 0 else calc_total

                st.markdown("---")
                f_col1, f_col2 = st.columns([2, 1])
                with f_col1:
                    # [แก้ไขจุดสำคัญ] ใส่ float() ครอบ display_total_value เพื่อแก้ Error MixedNumericTypesError
                    total_yuan_input = st.number_input("ราคาหยวนทั้งหมด *", min_value=0.0, step=0.0, format="%.2f", value=float(display_total_value), placeholder="0.00")
